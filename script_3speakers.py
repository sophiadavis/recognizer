...
(consisting of one excerpted recording from all speeches for each language), then randomly 
picks (n_reps) of these permutations. For each one, it trains a model using only three of  
the four recordings, then tests the model on a recording by the speaker whose voice was not 
...
def main():
    ...
    n_reps = 40 # number of random permutations of training data to run
